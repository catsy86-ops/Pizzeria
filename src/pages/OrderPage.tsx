...
const STATUSES = [
  ...
  { label: 'Wyrabianie', desc: 'Szef Fisz w akcji', icon: ChefHat, color: 'text-orange-400' },
...
export default function OrderPage() {
  ...
  return (
    <div className="py-12 px-6 max-w-7xl mx-auto space-y-12 pb-40">
      <header className="flex flex-col md:flex-row justify-between items-end gap-8 border-b border-white/5 pb-12">
        <div className="space-y-4">
          <motion.div 
            initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
            className="inline-flex items-center gap-2 bg-emerald-500/10 text-emerald-500 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border border-emerald-500/20"
          >
      ...
      <div className="flex flex-col lg:flex-row gap-12">
        ...
        <div className="flex-grow space-y-8">
          <motion.div 
            ...
            className="glass-premium rounded-[3.5rem] overflow-hidden shadow-2xl"
          >
            <div className="p-12 flex flex-col sm:flex-row justify-between items-center gap-10">
              <div className="text-center sm:text-left">
                ...
                <h2 className="text-8xl font-black text-white mt-4 tracking-tighter tabular-nums">{formatTime(timeLeft)}</h2>
              </div>
              <div className="w-full sm:w-auto glass-premium p-8 rounded-[2.5rem] flex items-center gap-6 border-white/5">
                 <div className="w-16 h-16 bg-primary rounded-2xl flex items-center justify-center text-white shadow-[0_0_30px_rgba(255,77,0,0.4)] animate-neon">
            ...
            {/* Nowoczesna Mapa Midnight */}
            <div className="h-96 bg-[#050505] relative overflow-hidden group border-y border-white/5">
               <div className="absolute inset-0 opacity-[0.05] grayscale bg-[url('https://www.transparenttextures.com/patterns/stardust.png')]" />
               
               <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-[85%] h-[2px] bg-white/5 rounded-full relative">
                     <motion.div 
                       initial={{ width: 0 }}
                       animate={{ width: `${(currentStep / (STATUSES.length - 1)) * 100}%` }}
                       className="absolute h-full bg-gradient-to-r from-primary to-accent rounded-full shadow-[0_0_20px_#ff4d00]" 
                     />
                     
                     <div className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-1/2 flex flex-col items-center">
                        <div className="w-14 h-14 glass-premium rounded-2xl flex items-center justify-center border-white/10 z-10 shadow-2xl">
                           <ChefHat size={24} className="text-white/40" />
                        </div>
                     </div>

                     <motion.div 
                       animate={{ left: `${(currentStep / (STATUSES.length - 1)) * 100}%` }}
                       className="absolute top-1/2 -translate-y-1/2 -translate-x-1/2 z-20"
                     >
                        <div className="bg-primary p-5 rounded-[1.5rem] shadow-[0_0_40px_rgba(255,77,0,0.6)] animate-neon">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                 ...
                 <div>
                    ...
                    <p className="text-white/40 font-medium text-lg italic">{STATUSES[currentStep].desc}</p>
                 </div>
              </motion.div>
            </AnimatePresence>
          </motion.div>
          ...
          <motion.div 
            ...
            className="glass-premium p-8 rounded-[3rem] flex flex-col sm:flex-row items-center justify-between gap-10"
          >
            ...
            <div className="flex gap-4 w-full sm:w-auto">
              <button className="flex-1 sm:flex-none p-6 glass-premium text-white rounded-[1.5rem] hover:bg-white hover:text-black transition-all active:scale-95 shadow-xl border-white/5">
                <MessageSquare size={24} />
              </button>
              <button className="flex-1 sm:flex-none p-6 bg-primary text-white rounded-[1.5rem] hover:bg-[#ff6a00] transition-all active:scale-95 shadow-2xl shadow-primary/20">
                <Phone size={24} />
              </button>
            </div>
          </motion.div>
          ...
          <div className="glass-premium rounded-[3.5rem] p-12 h-full shadow-2xl border-white/5">
            ...
            <div className="space-y-12 relative">
              ...
              {STATUSES.map((status, index) => {
                ...
                return (
                  <div key={status.label} className={`flex items-start gap-8 relative transition-all duration-700 ${active ? 'opacity-100' : 'opacity-10'}`}>
                    ...
                    <div className="pt-2">
                      <h4 className={`text-sm font-black uppercase tracking-widest ${active ? 'text-white' : 'text-white/20'}`}>
                        {status.label}
                      </h4>
            ...
            <div className="mt-20 pt-10 border-t border-white/5">
               <div className="bg-white/5 rounded-[2.5rem] p-8 border border-white/5 shadow-inner">
                  ...
                  <button className="w-full py-4 glass-premium text-white text-[10px] font-black uppercase tracking-[0.3em] rounded-2xl hover:bg-red-500/10 transition-colors border-red-500/20 text-red-500">
                     Potrzebujesz pomocy?
                  </button>
               </div>
            </div>
